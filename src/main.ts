...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    .setTitle(env.APP_NAME || 'Imo State Ministry of Finance TAAS CDR')
    .setDescription(env.APP_DESC || 'Central Data Reference CRUD/Microservice')
    .setVersion(env.APP_VER || '1.0')
    .addTag(env.APP_TAG || 'IMSG TAAS CDR')
    .addBearerAuth()
    .build();
